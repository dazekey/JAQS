...
end_date   20171001
"""
from __future__ import print_function  # 引用py 3.x的打印模块
from __future__ import absolute_import  # 绝对引入，优先从系统自带库引入模块
import time  # 引用时间模块

from jaqs.data import RemoteDataService  # 打开jaqs的远程数据模块
...
from alpha.config_path import DATA_CONFIG_PATH, TRADE_CONFIG_PATH
...
def test_save_dataview():
    ds = RemoteDataService()  # 打开远程服务器端口
...
if __name__ == "__main__":
    ...
    t3 = time.time() - t_start  # 结束时间
    print("\n\n\nTime lapsed in total: {:.1f}".format(t3))  # 打印出结束时间
